---
title: AdJ_5.7_-_Aprendiendo_de_Jesus_5.7
date: 2015-10-15
tags:
---
...
* Parches de seguridad del sistema base hasta 13.Oct.2015, que cierran las 10 fallas de seguridad y las 6 de robustez resueltas para fuentes de OpenBSD en http://www.openbsd.org/errata57.html. Los binarios distribuidos de OpenBSD 5.7 no resuelven estas fallas.
...
** Red: Fibra:   Ethernet:  Nuevo ```nep``` que soporta Sun Neptune 10GB, mejorado ```myx``` que soporta yricom Myri-10G PCI Express, mejorado ```msk``` para soporta Yukon Prime, Yukon Optima 2, Yukon 88E8079,  y varios EC U y chipsets Supreme; mejorado ```bge``` para soportar frames jumbo; mejorado ```sk``` que soporta XMAC II y Marvell Yukon 10/100G.  Inalámbrico:  Nuevo ```iwm``` que soporta dispositivos Intel 7200 IEEE 802.11a/ac/b/g/n; mejorado ```athn``` para soprotar Atheros UB94; mejorado ```win```. Inalámbrico USB: . Modem USB: Mejorado ```umsm``` para soportar MEDION S4222, mejorado ```umodem``` para soportar dispositivos Arduino Leonardo.
...
* Otros
** mandoc ahora soporta UTF-8 especificado con -K y a salida por defecto ahora es -Tlocale en lugar -Tascii
** syslogd puede enviar mensajes por TLS
** Nueva rcctl para controlar servicios
** Diversos programas pasados de base a portes: Sendmail, nginx.  Estos están disponibles como paquetes.


* El sistema base incluye mejoras a componentes auditados y mejorados como ```Xenocara``` (Xorg 7.7), ```gcc``` 4.2.1, ```perl``` 5.18.2, LibreSSL 2.0 con parches posteriores, OpenSMTP 5.4.3, nsd 4.0.3
...
Desde 5.7 el instalador no incluye etc ni xetc pues hacen part de base y xbase
...
* ruby 2.2.3 retroportado de OpenBSD-current y probado con aplicaciones Rails 4.2.4 Ver http://dhobsd.pasosdeJesus.org/Ruby_on_Rails_en_OpenBSD.html 
...
* Para cerrar fallas se han recompilado los siguientes paquetes a partir de portes actualizados de OpenBSD 5.7 (pero no incluidos en binarios de ese sistema): cups, cups-libs, cups-filters, curl, gdk-pixbuf, ghostscript, gnutls, icu4c, libksba, libtasn1, libwmf, libxml, net-snmp, netpbm, p5-Mail-!SpamAssassin, qemu, polkit, php-5.5.30, qemu, rrdtool
...
* Los paquetes exclusivos los encuentra en [http://adJ.pasosdeJesus.org/pub/AprendiendoDeJesus/5.7-amd64/paquetes] y otras extensiones de PostgreSQL y PHP que no hacen parte de la distribución en [http://adJ.pasosdeJesus.org/pub/AprendiendoDeJesus/5.7-amd64-paquetes-extra] 
...
* Se incluyen en total 517 paquetes, en los repositorios de paquetes para OpenBSD 5.7 hay 8588 disponibles para amd64
